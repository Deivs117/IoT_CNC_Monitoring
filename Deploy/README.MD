...
# 26. Configuración de alertas Telegram en Azure Functions

La función `telemetry_processor` evalúa la telemetría recibida desde IoT Hub y envía
alertas por Telegram cuando se detectan condiciones anómalas. Para que estas alertas
funcionen es necesario configurar dos variables de entorno en el Function App.
...
|---|---|---|
| `TELEGRAM_BOT_TOKEN` | Token del bot de Telegram (obtenido desde `@BotFather`) | Sí |
| `TELEGRAM_CHAT_ID` | ID del chat o grupo donde se enviarán las alertas | Sí |

Si alguna de las dos variables está ausente o vacía, el módulo `shared_code/alerts.py`
silencia las alertas sin lanzar excepciones, de modo que el resto del pipeline
(persistencia en Cosmos DB, etc.) sigue funcionando con normalidad.

---

## Cuándo se envía una alerta

La lógica de evaluación (`evaluate_alert` en `shared_code/alerts.py`) genera una alerta
cuando se cumple al menos una de estas condiciones:

- La temperatura está fuera del rango `[TEMP_MIN, TEMP_MAX]` (por defecto 15–45 °C).
- La humedad está fuera del rango `[HUM_MIN, HUM_MAX]` (por defecto 20–80 %).
- El estado vibracional reportado no es `normal`, `reposo` ni `desconocido`.
- El puntaje de anomalía vibracional supera `VIBRATION_ANOMALY_THRESHOLD` (por defecto 0.80).

Estos umbrales también se inyectan como App Settings y pueden ajustarse sin redeploying código.

---

## Opción 1 — Inyección automática mediante `02_backend.sh`

El script `Deploy/02_backend.sh` inyecta `TELEGRAM_BOT_TOKEN` y `TELEGRAM_CHAT_ID`
automáticamente en los App Settings del Function App, siempre que estén cargados
en la sesión antes de ejecutarlo.

### Paso 1 — Definir las variables en el archivo de secretos

```bash
# Editar ~/iot_cnc_secrets.env y agregar (o verificar) estas líneas:
export TELEGRAM_BOT_TOKEN="<token_obtenido_de_BotFather>"
export TELEGRAM_CHAT_ID="<id_del_chat_o_grupo>"
```

### Paso 2 — Cargar las variables en la sesión

```bash
source ~/iot_cnc_secrets.env
```

### Paso 3 — Ejecutar el script de backend

```bash
bash Deploy/02_backend.sh
```

El script inyecta ambas variables en los App Settings con el comando:
...
## Opción 2 — Configuración manual desde el Portal de Azure

1. Abre [portal.azure.com](https://portal.azure.com).
2. Navega a: **Function App → `cnc-iot-func` → Configuración → Variables de entorno**.
3. Haz clic en **+ Agregar** para cada variable:
   - Nombre: `TELEGRAM_BOT_TOKEN` / Valor: `<tu_token>`
   - Nombre: `TELEGRAM_CHAT_ID`  / Valor: `<tu_chat_id>`
4. Haz clic en **Guardar** y confirma el reinicio del Function App.

---

## Opción 3 — CLI manual (Azure Cloud Shell)

```bash
# Ajusta FUNC_APP_NAME y RG_NAME según tu entorno
...
### Listar todos los App Settings del Function App

```bash
az functionapp config appsettings list \
  --name cnc-iot-func \
  --resource-group rg-cnc-iot \
  --query "[?name=='TELEGRAM_BOT_TOKEN' || name=='TELEGRAM_CHAT_ID']" \
  --output table
```

La salida esperada muestra ambas variables con sus valores.

### Verificar desde los logs en tiempo real

```bash
func azure functionapp logstream cnc-iot-func
```

Cuando llegue un evento con anomalía, los logs mostrarán:
```
[telemetry_processor] Documento guardado en Cosmos DB: <id>
```
y la alerta llegará al chat de Telegram configurado.
...
curl -s -X POST \
  ...
  -d "{\"chat_id\": \"${TELEGRAM_CHAT_ID}\", \"text\": \"✅ Prueba de conectividad CNC Monitor\"}"
```

Una respuesta con `"ok": true` confirma que el token y el chat ID son válidos.

---

## Dónde vive la lógica de Telegram en el código

| Archivo | Responsabilidad |
|---|---|
| `backend/azure_functions/shared_code/alerts.py` | `evaluate_alert()`, `compose_telegram_message()`, `send_telegram_alert()` |
| `backend/azure_functions/telemetry_processor/__init__.py` | Llama a las funciones anteriores al procesar cada evento |
| `backend/azure_functions/requirements.txt` | Declara la dependencia `requests` necesaria para el envío HTTP |
